...
const MovieDetails = ({ movieData }) => {
  ...
  // Trabajos destacados
  const selectedJobs = ["Director", "Screenplay", "Story", "Screenplay", "Characters"];
  const mainCast =
    movieCast &&
  ...
  // Funcion para pasar el tiempo de la pelicula al formato correcto
  ...
  return (
    <div className="movie">
      ...
      <p className="movie__info">
        {new Date(movieData?.release_date).toLocaleDateString(language, { year: "numeric", month: "short", day: "numeric" })} ({language.slice(3)}) |{" "}
        ...
        | {formatMinutes(movieData?.runtime)}
      </p>
